...
it('sets the element at an index to the specified value', () => {
  ...
  expect(setAt(-4, 999, arr)).toEqual([1, 2, 3])
  ...
  expect(setAt(3, 999, arr)).toEqual([1, 2, 3])
})

it('removes the element if the new value is undefined', () => {
  const arr = [1, 2, 3]

  expect(setAt(-4, undefined, arr)).toEqual([1, 2, 3])
  ...
  expect(setAt(3, undefined, arr)).toEqual([1, 2, 3])
})
...
it('throws an error if the target is not an array', () => {
  ...
  expect(() => setAt(0, 1, {})).toThrowError(TypeError)
  expect(() => setAt(0, 1, '')).toThrowError(TypeError)
})
